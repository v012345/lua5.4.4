...
static lua_Number lua_strx2number(const char* s, char** endptr) {
    ...
    *endptr = cast_charp(s); /* valid up to here */
    ...
    if (*s == 'p' || *s == 'P') { /* exponent part? */
        ...
        s++; /* skip 'p' */
        neg1 = isneg(&s); /* sign */
        if (!lisdigit(cast_uchar(*s))) return l_mathop(0.0); /* invalid; must have at least one digit */
...
/// @brief 把数字转成字符串, buff 指向字符串, 使用 sprintf 实现; Convert a number object to a string, adding it to a buffer
...
/// @brief Convert a number object to a Lua string, replacing the value at 'obj'
