...
// o 是不是一个有效的索引, 只要 o 不是 nilvalue, o 就有效 \r
...
LUA_API void lua_settop(lua_State* L, int idx) {
    ...
    if (idx >= 0) {
        ...
        for (; diff > 0; diff--) setnilvalue(s2v(L->top++)); /* clear new slots */
    } else {
...
LUA_API void lua_closeslot(lua_State* L, int idx) {
    ...
    api_check(L, hastocloseCfunc(L->ci->nresults) && L->tbclist == level, "no variable to close at given level");
...
LUA_API const void* lua_topointer(lua_State* L, int idx) {
    ...
    switch (ttypetag(o)) {
        ...
        case LUA_VUSERDATA:
        case LUA_VLIGHTUSERDATA: return touserdata(o);
        default: {
            if (iscollectable(o))
                return gcvalue(o);
            else
 ...
 * @return LUA_API
 */
LUA_API void lua_rawset(lua_State* L, int idx) { aux_rawset(L, idx, s2v(L->top - 2), 2); }
...
LUA_API int lua_gc(lua_State* L, int what, ...) {
    ...
    switch (what) {
        ...
        case LUA_GCGEN: {
            ...
            if (majormul != 0) setgcparam(g->genmajormul, majormul);
        ...
        case LUA_GCINC: {
            ...
            res = isdecGCmodegen(g) ? LUA_GCGEN : LUA_GCINC;
            if (pause != 0) setgcparam(g->gcpause, pause);
            if (stepmul != 0) setgcparam(g->gcstepmul, stepmul);
            if (stepsize != 0) g->gcstepsize = stepsize;
...
static const char* aux_upvalue(TValue* fi, int n, TValue** val, GCObject** owner) {
    switch (ttypetag(fi)) {
        case LUA_VCCL: { /* C closure */
            ...
            if (!(cast_uint(n) - 1u < cast_uint(f->nupvalues))) return NULL; /* 'n' not in [1, f->nupvalues] */
            *val = &f->upvalue[n - 1];
        ...
        case LUA_VLCL: { /* Lua closure */
            ...
            TString* name;
            Proto* p = f->p;
            if (!(cast_uint(n) - 1u < cast_uint(p->sizeupvalues))) return NULL; /* 'n' not in [1, p->sizeupvalues] */
            *val = f->upvals[n - 1]->v;
...
LUA_API void* lua_upvalueid(lua_State* L, int fidx, int n) {
    TValue* fi = index2value(L, fidx);
    switch (ttypetag(fi)) {
        case LUA_VLCL: { /* lua closure */ return *getupvalref(L, fidx, n, NULL);
        }
        case LUA_VCCL: { /* C closure */
            CClosure* f = clCvalue(fi);
            if (1 <= n && n <= f->nupvalues) return &f->upvalue[n - 1];
            /* else */
        } /* FALLTHROUGH */
        case LUA_VLCF: return NULL; /* light C functions have no upvalues */
        default: {
