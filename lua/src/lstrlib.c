...
static int str_dump(lua_State* L) {
    ...
    state.init = 0;
    if (l_unlikely(lua_dump(L, writer, &state, strip) != 0)) return luaL_error(L, "unable to dump given function");
...
static void trymt(lua_State* L, const char* mtname) {
    ...
    if (l_unlikely(lua_type(L, 2) == LUA_TSTRING || !luaL_getmetafield(L, 2, mtname))) luaL_error(L, "attempt to %s a '%s' with a '%s'", mtname + 2, luaL_typename(L, -2), luaL_typename(L, -1));
...
static int check_capture(MatchState* ms, int l) {
    l -= '1';
    if (l_unlikely(l < 0 || l >= ms->level || ms->capture[l].len == CAP_UNFINISHED)) return luaL_error(ms->L, "invalid capture index %%%d", l + 1);
    return l;
}
...
static const char* classend(MatchState* ms, const char* p) {
    switch (*p++) {
        case L_ESC: {
            if (l_unlikely(p == ms->p_end)) luaL_error(ms->L, "malformed pattern (ends with '%%')");
            return p + 1;
        }
        case '[': {
            if (*p == '^') p++;
            do { /* look for a ']' */
                if (l_unlikely(p == ms->p_end)) luaL_error(ms->L, "malformed pattern (missing ']')");
...
static const char* matchbalance(MatchState* ms, const char* s, const char* p) {
    if (l_unlikely(p >= ms->p_end - 1)) luaL_error(ms->L, "malformed pattern (missing arguments to '%%b')");
    if (*s != *p)
        return NULL;
    else {
        int b = *p;
...
static const char* match(MatchState* ms, const char* s, const char* p) {
    if (l_unlikely(ms->matchdepth-- == 0)) luaL_error(ms->L, "pattern too complex");
            ...
            case L_ESC: { /* escaped sequences not in the format class[*+?-]? */
                switch (*(p + 1)) {
                    ...
                    case 'f': { /* frontier? */
                        ...
                        char previous;
                        p += 2;
                        if (l_unlikely(*p != '[')) luaL_error(ms->L, "missing '[' after '%%f' in pattern");
                ...
                if (!singlematch(ms, s, p, ep)) {
                    ...
                    switch (*ep) { /* handle optional suffix */
                        ...
                        case '+': /* 1 or more repetitions */
                            ...
                            /* FALLTHROUGH */
                        case '*': /* 0 or more repetitions */ s = max_expand(ms, s, p, ep); break;
                        case '-': /* 0 or more repetitions (minimum) */ s = min_expand(ms, s, p, ep); break;
                        default: /* no suffix */
                            s++;
                            p = ep;
...
static size_t get_onecapture(MatchState* ms, int i, const char* s, const char* e, const char** cap) {
    if (i >= ms->level) {
        if (l_unlikely(i != 0)) luaL_error(ms->L, "invalid capture index %%%d", i + 1);
        *cap = s;
        return e - s;
    } else {
...
static void push_onecapture(MatchState* ms, int i, const char* s, const char* e) {
    ...
    if (l != CAP_POSITION) lua_pushlstring(ms->L, cap, l);
...
static const char* getformat(lua_State* L, const char* strfrmt, char* form) {
    ...
    if (len >= MAX_FORMAT - 10) luaL_error(L, "invalid format (too long)");
    *(form++) = '%';
...
static int str_format(lua_State* L) {
    ...
    while (strfrmt < strfrmt_end) {
        if (*strfrmt != L_ESC)
            ...
            if (++arg > top) return luaL_argerror(L, arg, "no value");
            ...
            switch (*strfrmt++) {
                ...
                case 'd':
                case 'i': flags = L_FMTFLAGSI; goto intcase;
                case 'u': flags = L_FMTFLAGSU; goto intcase;
                case 'o':
                case 'x':
                case 'X':
                    flags = L_FMTFLAGSX;
                ...
                default: { /* also treat cases 'pnLlh' */ return luaL_error(L, "invalid conversion '%s' to 'format'", form);
                }
            }
...
static int getnum(const char** fmt, int df) {
    if (!digit(**fmt)) /* no number? */
        ...
        int a = 0;
        do { a = a * 10 + (*((*fmt)++) - '0'); } while (digit(**fmt) && a <= ((int)MAXSIZE - 9) / 10);
        return a;
    }
}
...
static int getnumlimit(Header* h, const char** fmt, int df) {
    ...
    if (l_unlikely(sz > MAXINTSIZE || sz <= 0)) return luaL_error(h->L, "integral size (%d) out of limits [1,%d]", sz, MAXINTSIZE);
    return sz;
}
...
static KOption getoption(Header* h, const char** fmt, int* size) {
    ...
    *size = 0; /* default */
    switch (opt) {
        ...
        case 'c':
            *size = getnum(fmt, -1);
            if (l_unlikely(*size == -1)) luaL_error(h->L, "missing size for format option 'c'");
            return Kchar;
        case 'z': return Kzstr;
...
static KOption getdetails(Header* h, size_t totalsize, const char** fmt, int* psize, int* ntoalign) {
    ...
    if (opt == Kpaddalign) { /* 'X' gets alignment from following option */
        if (**fmt == '\0' || getoption(h, fmt, &align) == Kchar || align == 0) luaL_argerror(h->L, 1, "invalid next option for option 'X'");
    }
...
static lua_Integer unpackint(lua_State* L, const char* str, int islittle, int size, int issigned) {
    ...
    if (size < SZINT) { /* real size smaller than lua_Integer? */
        ...
        for (i = limit; i < size; i++) {
            if (l_unlikely((unsigned char)str[islittle ? i : size - 1 - i] != mask)) luaL_error(L, "%d-byte integer does not fit into Lua Integer", size);
        }
    }
