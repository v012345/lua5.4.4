...
static int luaB_warn(lua_State* L) {
    ...
    for (i = 2; i <= n; i++) luaL_checkstring(L, i); /* make sure all arguments are strings */
...
static int luaB_tonumber(lua_State* L) {
    if (lua_isnoneornil(L, 2)) { /* standard conversion? */
        if (lua_type(L, 1) == LUA_TNUMBER) { /* already a number? */
            ...
            if (s != NULL && lua_stringtonumber(L, s) == l + 1) return 1; /* successful conversion to number */
            /* else not a number */
...
static int luaB_dofile(lua_State* L) {
    const char* fname = luaL_optstring(L, 1, NULL);
    lua_settop(L, 1);
    if (l_unlikely(luaL_loadfile(L, fname) != LUA_OK)) return lua_error(L);
...
/// @brief
/// Do a protected call with error handling. After 'lua_rotate', the stack will have <f, err, true, f, [args...]>;
/// so, the function passes 2 to 'finishpcall' to skip the 2 first values when returning results.
/// @param L
/// @return
