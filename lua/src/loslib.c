...
static int os_tmpname(lua_State* L) {
    ...
    int err;
    lua_tmpnam(buff, err);
    if (l_unlikely(err)) return luaL_error(L, "unable to generate a unique filename");
...
static void setfield(lua_State* L, const char* key, int value, int delta) {
    ...
    if (l_unlikely(value > LUA_MAXINTEGER - delta)) luaL_error(L, "field '%s' is out-of-bound", key);
#endif
...
static int getfield(lua_State* L, const char* key, int d, int delta) {
    ...
    if (!isnum) { /* field is not an integer? */
        ...
        if (!(res >= 0 ? (lua_Unsigned)res <= (lua_Unsigned)INT_MAX + delta : (lua_Integer)INT_MIN + delta <= res)) return luaL_error(L, "field '%s' is out-of-bound", key);
        res -= delta;
    }
    lua_pop(L, 1);
    return (int)res;
...
static int os_time(lua_State* L) {
    ...
    if (t != (time_t)(l_timet)t || t == (time_t)(-1)) return luaL_error(L, "time result cannot be represented in this installation");
    l_pushtime(L, t);
    return 1;
}
