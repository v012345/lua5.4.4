...
/// @brief 在分配内存的时候, 根据元素的大小来限制分配元素的个数, 具体来说, 如何用户指定的数 n 小于 MAX_SIZET/元素大小, 就返回 n, 否则使用 MAX_SIZET/元素大小 \r
/// Computes the minimum between 'n' and 'MAX_SIZET/sizeof(t)', so that the result is not larger than 'n' and cannot overflow a 'size_t'
/// when multiplied by the size of type 't'. (Assumes that 'n' is an 'int' or 'unsigned int' and that 'int' is not larger than 'size_t'.)
