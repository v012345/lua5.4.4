...
void luaV_finishOp(lua_State* L) {
    ...
    switch (op) { /* finish its execution */
        ...
        case OP_GTI:
        case OP_GEI:
        case OP_EQ: { /* note that 'OP_EQI'/'OP_EQK' cannot yield */ int res = !l_isfalse(s2v(L->top.p - 1)); L->top.p--;
        ...
        vmdispatch(GET_OPCODE(i)) {
            ...
            vmcase(OP_RETURN0) {
                if (l_unlikely(L->hookmask)) {
                    ...
                    L->top.p = base - 1;
                    for (nres = ci->nresults; l_unlikely(nres > 0); nres--) setnilvalue(s2v(L->top.p++)); /* all results are nil */
                }
                goto ret;
            }
            vmcase(OP_RETURN1) {
                if (l_unlikely(L->hookmask)) {
                    ...
                    if (nres == 0)
                        ...
                        L->top.p = base;
                        for (; l_unlikely(nres > 1); nres--) setnilvalue(s2v(L->top.p++)); /* complete missing results */
                    }
                }
