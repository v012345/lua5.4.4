...
local OP_ACT = {
    OP_MOVE = function(code)
        local f = "%s R[%s] => R[%s]"
        local name = OP_CODE[(code & 0x7F) + 1]
        local B = Bytedump:B(code)
        local A = Bytedump:A(code)
        print(string.format(f, name, B, A))
    end,
    OP_LOADI = function(code)
        local f = "%s sBx:%s => R[%s]"
        local name = OP_CODE[(code & 0x7F) + 1]
        local sBx = Bytedump:sBx(code)
        local A = Bytedump:A(code)
        print(string.format(f, name, sBx, A))
    end,
    OP_LOADF = nil,
    OP_LOADK = function(code)
        local f = "%s K[%s] => R[%s]"
        local name = OP_CODE[(code & 0x7F) + 1]
        local A = Bytedump:A(code)
        local B = Bytedump:B(code)
    ...
    OP_LOADKX = nil,
    OP_LOADFALSE = function(code)
        local f = "%s false => R[%s]"
        local name = OP_CODE[(code & 0x7F) + 1]
        local A = Bytedump:A(code)
        print(string.format(f, name, A))
    end,
    OP_LFALSESKIP = nil,
    OP_LOADTRUE = function(code)
        local f = "%s true => R[%s]"
        local name = OP_CODE[(code & 0x7F) + 1]
        local A = Bytedump:A(code)
        print(string.format(f, name, A))
    ...
    OP_GETUPVAL = nil,
    OP_SETUPVAL = nil,
    OP_GETTABUP = function(code)
        local f = "%s UpValue[%s][K[%s]] => R[%s]"
        local name = OP_CODE[(code & 0x7F) + 1]
        local A = Bytedump:A(code)
        local B = Bytedump:B(code)
        local C = Bytedump:C(code)
        print(string.format(f, name, B, C, A))
    end,
    OP_GETTABLE = nil,
    OP_GETI = nil,
    OP_GETFIELD = nil,
    ...
    OP_TEST = nil,
    OP_TESTSET = nil,
    OP_CALL = function(code)
        local f = "%s R[%s](arg * B:%s) {return * (C:%s - 1)}"
        local name = OP_CODE[(code & 0x7F) + 1]
        local A = Bytedump:A(code)
        local B = Bytedump:B(code)
        local C = Bytedump:C(code)
        print(string.format(f, name, A, B, C))
    end,
    OP_TAILCALL = nil,
    OP_RETURN = nil,
    OP_RETURN0 = nil,
...
for _, file in pairs(GetOpCodes()) do
    print(file.script_name)
    for _, code in pairs(file) do
